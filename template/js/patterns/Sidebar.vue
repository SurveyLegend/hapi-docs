    ...
    <div class="sidebar">
        <nav role="navigation" class="sidebar__navigation">
            ...
            <ul class="sidebar__navigation__items" v-for="(group, groupName) in routes">
                <li>
                    <a class="sidebar__navigation__item" :href="`#${groupName}`" @click="$parent.scrollTo(`#${groupName}`)">{{ groupName | capitalize }}</a>
                    <ul class="sidebar__navigation__items">
                        <li v-for="route in group">
                            <a class="sidebar__navigation__item" :href="`#${route.slug}`" @click="$parent.scrollTo(`#${route.slug}`)">{{ route.description }}</a>
                        </li>
                    </ul>
                </li>
            </ul>
