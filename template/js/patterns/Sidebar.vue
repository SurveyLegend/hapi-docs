    ...
    <div class="sidebar">
        <nav role="navigation" class="sidebar__navigation">
            ...
            <ul class="sidebar__navigation__items" v-for="(group, groupName) in routes">
                <li>
                    <a class="sidebar__navigation__item" :href="`#${groupName}`" v-scroll-to="`#${groupName}`">{{ groupName | capitalize }}</a>
                    <ul class="sidebar__navigation__items">
                        <li v-for="route in group">
                            <a class="sidebar__navigation__item" :href="`#${route.slug}`" v-scroll-to="`#${route.slug}`">{{ route.description }}</a>
                        </li>
                    </ul>
                </li>
            </ul>
